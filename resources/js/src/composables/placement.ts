...
export function usePlacement(boardSize = 12, fleet: ShipSpec[]) {
  ...
  function randomlyPlaceAll() {
    reset()
    while (nextIndex.value < palette.length) {
      const size = palette[nextIndex.value]
      let placed = false
      while (!placed) {
        const dir: Dir = Math.random() < 0.5 ? 'H' : 'V'
        const x = Math.floor(Math.random() * boardSize)
        const y = Math.floor(Math.random() * boardSize)
        if (canPlace(x, y, size, dir)) {
          applyShip(x, y, size, dir)
          placed = true
        }
      }
    }
  ...
  return { board, canPlace, applyShip, removeLast, reset, randomlyPlaceAll, allPlaced, nextSize, nextIndex, orientation, placedShips, palette }
}

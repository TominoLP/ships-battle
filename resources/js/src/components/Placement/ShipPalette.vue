...
const orientationLabel = computed(() =>
	props.orientation === 'H'
		? t('placing.orientation.horizontal')
		: t('placing.orientation.vertical')
);
		...
		<div class="flex flex-col gap-2 mt-5">
			<button class="group inline-flex items-center justify-center gap-2 rounded-lg border px-3 py-2
                     border-slate-600 bg-slate-800 hover:bg-slate-700 text-slate-200"
							@click="$emit('toggleOrientation')" :title="t('placing.rotateTitle')">
				<i class="fa-solid fa-arrows-rotate text-sm transition-transform group-hover:rotate-90"></i>
				<span class="text-xs sm:text-[13px]">{{ t('placing.rotate') }}</span>
				<span class="hidden sm:inline text-[10px] opacity-60">{{ t('placing.rotateShortcut') }}</span>
			</button>

			<button class="inline-flex items-center justify-center gap-2 rounded-lg border px-3 py-2
                     border-slate-600 bg-slate-800 hover:bg-slate-700 text-slate-200"
							@click="$emit('undo')" :title="t('common.undoTitle')">
				<i class="fa-solid fa-rotate-left text-sm"></i>
				<span class="text-xs sm:text-[13px]">{{ t('common.undo') }}</span>
				<span class="hidden sm:inline text-[10px] opacity-60">{{ t('common.undoShortcut') }}</span>
			</button>
		</div>
	</div>
</template>

...
const tabs: Array<{ key: TabKey; label: string }> = [
	{ key: 'ships', label: t('leaderboard.tabs.ships') },
	{ key: 'wins', label: t('leaderboard.tabs.wins') },
];
...
const valueTitle = computed(() =>
	activeTab.value === 'wins'
		? t('leaderboard.valueTitle.wins')
		: t('leaderboard.valueTitle.ships')
);
...
async function load(metric: TabKey) {
	...
	try {
		...
		leaderboard.value = [];
		error.value = err instanceof Error ? err.message : t('leaderboard.errorLoad');
	} finally {
		if (token === requestToken) loading.value = false;
	}
}

watch(activeTab, (metric) => void load(metric), { immediate: true });

const refreshHandler = () => void load(activeTab.value);
...
<template>
	<div class="rounded-xl border border-slate-700/70 bg-slate-900/70 shadow-lg overflow-hidden">
		...
		<div class="flex flex-col gap-3 border-b border-slate-700/60 px-4 py-4 sm:flex-row sm:items-center">
			<h2 class="text-base font-semibold text-slate-100 flex-1">{{ t('leaderboard.title') }}</h2>
		...
		<div class="px-4 py-4 text-sm text-slate-200">
			<Transition name="fade-slide" mode="out-in">
				<div :key="`${activeTab}-${loading}-${error}-${leaderboard.length}`">
					<div v-if="loading" class="py-6 text-center text-slate-400">{{ t('leaderboard.loading') }}</div>
					<div v-else-if="error" class="py-6 text-center text-rose-300">{{ error }}</div>
					<div v-else-if="leaderboard.length === 0" class="py-6 text-center text-slate-400">{{ t('leaderboard.empty') }}</div>
					...
					<div v-else class="overflow-x-auto">
						<table class="w-full text-left text-xs sm:text-sm border-collapse">
							...
							<tr>
								...
								<th class="py-2 px-3">{{ t('leaderboard.columns.player') }}</th>
								<th class="py-2 px-3">{{ valueTitle }}</th>
								<th v-if="activeTab === 'ships'" class="py-2 px-3">{{ t('leaderboard.columns.avgPerGame') }}</th>
								<th v-if="activeTab !== 'ships'" class="py-2 px-3">{{ t('leaderboard.columns.winrate') }}</th>
								<th class="py-2 px-3 text-right">{{ t('leaderboard.columns.games') }}</th>
							</tr>
							</thead>
...
.fade-slide-move { transition: transform 0.25s ease; }
</style>

...
const levelLabel = computed(() => {
	const lvl = props.level;
	if (!lvl) return t('progress.level');
	return `${lvl.current?.name ?? t('progress.level')}`;
});

function achievementStatus(a: AchievementItem): string {
	if (a.type === 'event') return a.progress.completed ? t('progress.unlocked') : t('progress.notUnlocked');
	if (a.progress.completed) return `${t('progress.completed')} (${a.progress.value})`;
...
type StepsLikeMap = Map<number, number>;
function isMap(s: StepsLike): s is StepsLikeMap {
...
function stepPoints(a: AchievementItem, th: number): number {
	const s = a.steps;
	if (isMap(s)) return s.get(th) ?? 0;
	const v = (s as Record<string, number>)[String(th)];
			...
			<div class="w-[min(1400px,92vw)] max-h-[88vh] overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl">
				<div class="flex items-center justify-between gap-3 border-b border-slate-700/70 px-5 py-4">
					<div class="flex items-center gap-3">
						...
						<h2 class="text-lg font-semibold text-slate-100">{{ t('progress.title') }}</h2>
					</div>
					<button
				...
				<div class="overflow-y-auto p-5">
					<div class="rounded-xl border border-slate-700/70 bg-slate-900/70 p-5">
						<div class="mb-3 flex flex-wrap items-end justify-between gap-4">
							<div>
								<div class="text-xs font-medium uppercase tracking-wide text-slate-400">{{ t('progress.currentRank') }}</div>
								<div class="mt-1 text-2xl font-bold text-slate-100">{{ levelLabel }}</div>
							</div>
							<div class="text-right">
								<div class="text-xs font-medium uppercase tracking-wide text-slate-400">{{ t('progress.totalPoints') }}</div>
						...
						<div class="mt-2.5 flex items-center justify-between text-xs text-slate-400">
							<div class="flex items-center gap-1.5">
								{{ props.level?.current?.name ?? t('progress.level') }}
								<span v-if="props.level?.current" class="opacity-60">• {{ props.level.current.min_points }} {{ t('progress.pointsShort') }}</span>
							</div>
							<div v-if="props.level?.next" class="font-medium">
								{{ props.level.next.name }}
								<span class="text-amber-400">• {{ props.level.next.points_to_go }} {{ t('progress.pointsShort') }}</span>
							</div>
							<div v-else class="font-medium text-amber-300">{{ t('progress.maxLevelReached') }}</div>
						</div>
					</div>

					<div class="mt-5 flex flex-wrap items-center justify-between gap-3">
						<div class="text-slate-300">{{ t('progress.achievements') }}</div>
					...
					<div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
						<div
							...
							:key="a.key"
							class="rounded-xl border border-slate-700 bg-slate-900/70 p-4 transition-shadow hover:shadow-lg hover:shadow-slate-900/50"
						>
                      ...
                      : (a.progress.completed ? 'fa-medal text-amber-300' : 'fa-flag text-slate-400')"
								...
								>
                  {{ a.type === 'event'
									? (a.progress.completed ? t('progress.unlocked') : t('progress.event'))
									: t('progress.tiers')
									}}
                </span>
							</div>
                    ...
                    +{{ a.event_points }}{{ t('progress.pointsShort') }}
                  </span>
								</div>
                  ...
                  <span class="text-slate-400">
                    {{ t('progress.nextStep') }}:
                    <strong class="text-slate-300">{{ a.progress.next_step - a.progress.value }}</strong>
                  </span>
									<span class="flex items-center gap-2 text-slate-400">
                    <span class="rounded-md bg-slate-800 px-1.5 py-0.5 text-[10px] font-medium text-emerald-400">
                      +{{ nextStepPoints(a) ?? 0 }}{{ t('progress.pointsShort') }}
                    </span>
                  </span>
								</div>
							</div>
                ...
                <span
									v-for="th in stepThresholdsSorted(a)"
									:key="th"
									class="rounded-full border px-2 py-0.5 text-[11px]"
									:class="(a.progress.highest_step ?? 0) >= th
                    ? 'border-emerald-500/60 text-emerald-400'
                    : 'border-slate-600 text-slate-400'"
									:title="t('progress.pointsAtThreshold', { points: stepPoints(a, th), threshold: th })"
								>
                  {{ th }}
                </span>
							</div>

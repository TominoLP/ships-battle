...
<template>
	<section class="space-y-4">
		<header class="flex items-start justify-between">
			<div class="space-y-1">
				<h3 class="text-lg font-semibold text-slate-100">{{ t('stats.title') }}</h3>
			</div>
			<div class="rounded-lg border border-slate-600 bg-slate-900/80 px-3 py-1 text-xs font-medium text-slate-200">
				<p class="text-xs font-mono text-slate-400">
					{{ t('stats.playTime') }} <span class="text-slate-200">{{ formattedTime }}</span>
				</p>
			</div>
		</header>
		...
		<div class="space-y-4 divide-y divide-slate-800 rounded-lg border border-slate-700 bg-slate-900/60">
			<div class="p-4">
				<h4 class="mb-3 text-sm font-semibold text-red-500 uppercase tracking-wide">
					{{ t('stats.enemyAttacks', { name: props.enemyName || t('stats.opponent') }) }}
				</h4>
				<dl class="space-y-2 text-sm text-slate-300">
					<div class="flex justify-between">
						<dt>{{ t('stats.labels.shotsFired') }}</dt>
					...
					<div class="flex justify-between border-t border-slate-800 pt-2">
						<dt>{{ t('stats.labels.yourIntactCells') }}</dt>

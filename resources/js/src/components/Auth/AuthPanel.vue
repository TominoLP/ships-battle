...
const canSubmit = computed(() => {
  return form.name.trim().length >= 3 && form.password.trim().length >= 6;
});
...
async function extractErrors(err: unknown): Promise<ParsedErrors> {
  const fallback = 'Etwas ist schiefgelaufen. Bitte versuche es erneut.';
  ...
  if (response) {
    try {
      ...
      if (data?.errors && typeof data.errors === 'object') {
        parsed.fields = {};
        if (Array.isArray(data.errors.name) && data.errors.name[0]) {
          parsed.fields.name = data.errors.name[0];
        }
        if (Array.isArray(data.errors.password) && data.errors.password[0]) {
          parsed.fields.password = data.errors.password[0];
        }
      }
      if (typeof data?.message === 'string' && !parsed.general) {
        parsed.general = data.message;
      }
    } catch {
      try {
        const text = await response.clone().text();
        if (text.trim().length && !parsed.general) {
          parsed.general = text.trim();
        }
      } catch {
        // ignore
      }
  ...
  if (anyErr?.message && typeof anyErr.message === 'string') {
    parsed.general = parsed.general ?? anyErr.message;
  }

  if (!parsed.general && !parsed.fields) {
    parsed.general = fallback;
  }
...
<template>
  <div class="max-w-md mx-auto rounded-xl border border-slate-700 bg-slate-900/70 p-6 shadow-lg backdrop-blur">
    ...
    <p class="text-sm text-slate-400 mb-6 text-center">
      Melde dich an, um Spiele zu erstellen, deinen Fortschritt zu speichern und Statistiken zu sammeln.
    </p>
    ...
    <form class="space-y-4" @submit.prevent="submit">
      ...
      <transition name="fade">
        ...
        >
          <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-rose-700/60 text-xs font-semibold">
            !
          </span>
          <span>{{ JSON.parse(fieldErrors.general.toString().replace(/^[^{]+/, '')).message }}</span>
        </div>
      </transition>

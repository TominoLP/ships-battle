...
const panels: Array<{
  key: 'create-private' | 'create-public' | 'join' | 'bot';
  title: string;
  description: string;
  icon: string;
  accent: string;
  ...
  },
  {
    ...
    action: () => {
      selectCreateType(true);
    },
  },
  {
    key: 'join',
    title: 'Code eingeben',
    description: 'Du hast bereits einen Code? Gib ihn ein und werde direkt Teil des Gefechts.',
...
function confirmJoin() {
  if (!canJoin.value || joinLoading.value) {
    return;
  }
  ...
  try {
    emit('join', {
      ...
      onError: (message: string) => {
        joinLoading.value = false;
        joinError.value = message || 'Game Code wurde nicht akzeptiert.';
        focusInput(0);
      },
    });
  } catch {
    joinLoading.value = false;
    joinError.value = 'Etwas ist schiefgelaufen. Bitte versuche es erneut.';
  }
}
...
function syncDigitsFromModel(value: string) {
  ...
  if (next.every((val, idx) => val === codeDigits.value[idx])) {
    return;
  }
...
<template>
  <section class="space-y-6">
    <header class="space-y-2">
      <h3 class="text-lg font-semibold text-slate-100">Wie möchtest du starten?</h3>
      <p class="text-sm text-slate-400">
        Wähle ein Szenario unten oder nutze den Game Code, wenn du bereits eingeladen wurdest.
      </p>
    </header>
    ...
    >
      ...
      <p class="text-sm text-slate-400">
        Der Code besteht aus 6 Zeichen. Wir überprüfen ihn automatisch, sobald du ihn vollständig eingegeben hast.
      </p>

      <div class="space-y-2">
        <label class="text-xs uppercase tracking-wide text-slate-400">Game Code</label>
        ...
        <transition name="fade">
          ...
          >
            ...
            <span>Der Spielcode ist ungültig oder das Spiel existiert nicht mehr.</span>
          </p>
        </transition>
      </div>
      ...
      >
        ...
        <span>{{ joinLoading ? 'Verbindung wird hergestellt…' : 'Spiel beitreten' }}</span>
      </button>
    </form>
    ...
    >
      <div class="flex items-center justify-between">
        <h4 class="text-base font-semibold text-slate-100">Spiel erstellen</h4>
        <button
          type="button"
          class="text-xs text-slate-400 hover:text-slate-200"
          @click="goBack"
      ...
      >
        <p class="font-semibold flex items-center gap-2">
          ...
          {{ makePublic ? 'Öffentliches Spiel' : 'Privates Spiel' }}
        </p>
        <p class="mt-1 text-xs opacity-90">
          <template v-if="makePublic">
            Dein Match erscheint in der öffentlichen Lobby. Andere Spieler können ohne Code einsteigen.
          </template>
          <template v-else>
            Nur Spieler mit deinem Code können beitreten. Teile ihn nach dem Erstellen mit deinem Mitspieler.
          </template>
        </p>
      </div>
